...
routes = [
    ...
    APIRoute(path='/book/', endpoint=post_book, methods=["POST"], response_model=Book, dependencies=[Depends(check_user_role)]),
    APIRoute(path='/shelf/', endpoint=get_shelf, methods=["GET"], response_model=Shelf, dependencies=[Depends(check_user_role)]),
    APIRoute(path='/shelf/{status}/', endpoint=get_books_by_status, methods=["GET"], response_model=Shelf, dependencies=[Depends(check_user_role)]),
    APIRoute(path="/book/{id}/add_to_shelf/", endpoint=add_book_to_shelf, methods=["PUT", "PATCH"], dependencies=[Depends(check_user_role)]),
    APIRoute(path="/shelf/{id}/", endpoint=get_book_from_shelf, response_model=BookEntry, methods=["GET"], dependencies=[Depends(check_user_role)]),
    APIRoute(path="/shelf/{id}/mark_as_read/", endpoint=mark_book_as_read, methods=["PUT", "PATCH"], dependencies=[Depends(check_user_role)]),
]
